...
class Sampler:
    def __init__(self, device, mask_ind, emb_f, batch_size, channel, auto_regre_num, timesteps, ddim_timesteps, cm_timesteps, **kwargs):
        ...
        self.add_object_voxel = kwargs.get('add_object_voxel', True)  # whether to add object voxels
    ...
    def consistency_loss(self, x_start, joints, mat, scene_flag, mask, t, text_emb, pelvis_goal, hand_goal, object_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, is_loco, is_object, obj_bps_data, obj_rot_mat_ref, rest_pose_obj_nn_pts, transformed_obj_verts, rest_human_offsets, object_points=None, noise=None, loss_type='l2'):
        ...
        if self.dataset.load_scene:
            with torch.no_grad():
                ...
                if not self.add_object_voxel:
                    occ = self.dataset.get_occ_for_points(query_points, None, scene_flag).float()
                else:
                ...
                if self.scene_type in ['plane_two', 'occ_two', 'occ_temp']:
                    ...
                    end_goal_pos[need_pelvis_dir] = pelvis_goal_copy[need_pelvis_dir].reshape(self.batch_size, 3)[:, [0, 2]]
                    end_goal_pos[is_object] = object_goal_copy[is_object].reshape(self.batch_size, 3)[:, [0, 2]]
                ...
                if self.scene_type == 'occ_temp':
                    ...
                    for i in temp_indices:
                        ...
                        if self.add_object_voxel:
                            occ_temp = self.dataset.get_occ_for_points(query_points, object_points_temp[:, i, :, :], scene_flag)
                        else:
                            occ_temp = self.dataset.get_occ_for_points(query_points, None, scene_flag)
        ...
        pred_x_0 = self.student_model(x_start_noisy, occ, start_timestep, text_emb, pelvis_goal, hand_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, object_goal, is_object, obj_bps_data, occ_list, occ_pos, cfg_scale=w)
        ...
        with torch.no_grad():
            ...
            cond_pred = self.teacher_model(x_start_noisy, occ, start_timestep, text_emb, pelvis_goal, hand_goal, 
                                          is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, 
            ...
            target_pred_x0 = self.target_model(x_prev, occ, timesteps, text_emb, pelvis_goal, hand_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, object_goal, is_object, obj_bps_data, occ_list, occ_pos)
        ...
        # loss_jpos = F.mse_loss(target[:, :, :84][mask_inv[:, :, :84]].float(), model_pred[:, :, :84][mask_inv[:, :, :84]].float())
        # loss_jrot = F.l1_loss(target[:, :, 84:216][mask_inv[:, :, 84:216]].float(), model_pred[:, :, 84:216][mask_inv[:, :, 84:216]].float())
        
        # loss_otrans = F.mse_loss(target[:, :, 216:219][mask_inv[:, :, 216:219]].float(), model_pred[:, :, 216:219][mask_inv[:, :, 216:219]].float())
        # loss_orot = F.l1_loss(target[:, :, 219:228][mask_inv[:, :, 219:228]].float(), model_pred[:, :, 219:228][mask_inv[:, :, 219:228]].float())
        
        # loss_contact = F.l1_loss(target[:, :, 228:232][mask_inv[:, :, 228:232]].float(), model_pred[:, :, 228:232][mask_inv[:, :, 228:232]].float())
    ...
    def cm_sample(self, model, x0, x, fixed_points, mat, scene_flag, t, t_index,
        ...
        if t_index > 0:
            ...
            model_output = model(x, occ, start_timestep, text_emb, pelvis_goal, hand_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, object_goal, is_object, obj_bps_data, occ_list, occ_pos, is_sample=True, is_uncondition=is_uncondition, cfg_scale=w)
            ...
            with torch.enable_grad():
                ...
                # is_penetrating, nearest_free_points = self.dataset.get_nearest_free_voxel(verts, scene_flag)
                # loss = F.mse_loss(verts, nearest_free_points) * 3000000
                # loss += F.mse_loss(verts[:, 1:], verts[:, :-1]) * 1000

                # is_penetrating, nearest_free_points = self.dataset.get_nearest_free_voxel(obj_verts, scene_flag)
                # loss += F.mse_loss(obj_verts, nearest_free_points) * 500000
                ...
                # x_prev[:, :, 216:] = x_prev[:, :, 216:] + penetration_gradient[:, :, 216:] * (1 - alpha_cumprod) * 0.00075
                # x_prev[:, :, :216] = x_prev[:, :, :216] + penetration_gradient[:, :, :216] * (1 - alpha_cumprod) * 0.005
            ...
            pred_x_0 = self.student_model(x, occ, start_timestep, text_emb, pelvis_goal, hand_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, object_goal, is_object, obj_bps_data, occ_list, occ_pos, is_sample=True, is_uncondition=is_uncondition, cfg_scale=w)
    ...
    def p_losses(self, x_start, joints, mat, scene_flag, mask, t, text_emb, pelvis_goal, hand_goal, object_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, is_loco, is_object, obj_bps_data, obj_rot_mat_ref, rest_pose_obj_nn_pts, transformed_obj_verts, rest_human_offsets, object_points, noise=None, loss_type='huber'):
        ...
        if self.dataset.load_scene:
            with torch.no_grad():
                # print(x_noisy.shape, joints.shape, mat.shape)
                x_orig = transform_points(self.dataset.denormalize_torch(x_noisy[:, :, :joints.shape[-1]]), mat)
                mat_for_query = mat.clone()
                target_ind = self.mask_ind if self.mask_ind != -1 else 0
                mat_for_query[:, :3, 3] = x_orig[:, self.emb_f, target_ind * 3: target_ind * 3 + 3]
                mat_for_query[:, 1, 3] = 0
                query_points = transform_points(self.grid, mat_for_query)
                if not self.add_object_voxel:
                    occ = self.dataset.get_occ_for_points(query_points, None, scene_flag)
                else:
                    occ = self.dataset.get_occ_for_points(query_points, object_points, scene_flag)
        ...
        predicted_noise = self.student_model(x_noisy, occ, t, text_emb, pelvis_goal, hand_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, object_goal, is_object, obj_bps_data, occ_list, occ_pos)
        ...
        # loss_jpos = F.mse_loss(x_start[:, :, :84], predicted_noise[:, :, :84])
        # loss_jrot = F.l1_loss(x_start[:, :, 84:216], predicted_noise[:, :, 84:216])
        
        # loss_otrans = F.mse_loss(x_start[:, :, 216:219], predicted_noise[:, :, 216:219])
        # loss_orot = F.l1_loss(x_start[:, :, 219:228], predicted_noise[:, :, 219:228])
        
        # loss_contact = F.l1_loss(x_start[:, :, 228:232], predicted_noise[:, :, 228:232])
    ...
    def p_sample_loop(self, fixed_points, mat, scene_flag, text_emb, pelvis_goal, hand_goal, object_goal, \
                    is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, is_loco, is_object, obj_bps_data, object_points, obj_rot_mat_ref, obj_rest_verts, seq_name_dict, obj_rot_mat_prefix=None, object_only=False):
        self.batch_size = fixed_points.shape[0]
        device = next(self.student_model.parameters()).device
        shape = (self.batch_size, self.dataset.max_window_size, self.channel)
        points = torch.randn(shape, device=device)
    ...
    def p_sample(self, model, x0, x, fixed_points, mat, scene_flag, t, t_index,
                 text_emb, pelvis_goal, hand_goal, object_goal, is_pick, need_scene,
                 need_pelvis_dir, pi, end_pi, seq_length, need_pi, is_loco, is_object, obj_bps_data, object_points, obj_rot_mat_ref, obj_rest_verts, seq_name_dict, obj_rot_mat_prefix, object_only=False):
        if self.dataset.load_scene:
            x_orig = transform_points(self.dataset.denormalize_torch(x[:, :, :84]), mat)
            mat_for_query = mat.clone()
            target_ind = self.mask_ind if self.mask_ind != -1 else 0
        ...
        # )
        cond_model_output = model(x, occ, t, text_emb, pelvis_goal, hand_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, object_goal, is_object, obj_bps_data, occ_list, occ_pos, is_sample=True)

        uncond_model_output = model(x, occ, t, text_emb, pelvis_goal, hand_goal, is_pick, need_scene, need_pelvis_dir, pi, end_pi, seq_length, need_pi, object_goal, is_object, obj_bps_data, occ_list, occ_pos, is_sample=True, is_uncondition=True)
