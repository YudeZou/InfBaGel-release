...
class CHOISEvaluationDataset(Dataset):
    def __init__(
        ...
        data_root_folder = "/cpfs04/shared/sport/zouyude/code/chois_release/processed_data/"
        self.language_anno_folder = os.path.join(data_root_folder, "omomo_text_anno_json_data") 
     
        # For jpos mean, std
        mean_std_jpos_path = os.path.join('/cpfs04/shared/sport/zouyude/code/lingo-release/train/joints_mean_std.npz')
        if os.path.exists(mean_std_jpos_path):
            mean_std_jpos_data = np.load(mean_std_jpos_path)
        
        self.mean_jpos = torch.from_numpy(mean_std_jpos_data['jpos_mean']).float() # 72
        self.std_jpos = torch.from_numpy(mean_std_jpos_data['jpos_std']).float() # 72
    ...
    def load_language_annotation(self, seq_name):
        ...
        if os.path.exists(os.path.join(self.language_anno_folder, seq_name+".json")):
            # Load json file 
            ...
            text_dict = {}
            text_dict['caption'] = text_anno
            # 将句子按标点符号分割,去除标点,转小写,最后分词
            text_dict['tokens'] = [f"{token.strip(',.').lower()}/OTHER" for token in text_anno.replace(',', ' ').split()]
        else:
            # Load .txt file 
    ...
    def __getitem__(self, index):  
        ...
        # data_input = self.normalize_jpos_mean_std(ori_jpos.reshape(-1, 24, 3)).reshape(-1, 24*3) # T X 72 
        ...
        # Add padding. 
        # new_data_input = new_data_input[-120:]
        actual_steps = new_data_input.shape[0]
        if actual_steps < 360: # max len: 336
            paded_new_data_input = torch.cat((new_data_input, torch.zeros(360-actual_steps, new_data_input.shape[-1])), dim=0)
